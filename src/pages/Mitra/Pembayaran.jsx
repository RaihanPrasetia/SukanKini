...
<div className="w-full bg-gray-50 p-6 py-24 lg:pt-32 rounded-lg shadow-lg min-h-[80vh]">
    ...
    <div className="overflow-x-auto bg-white shadow-lg sm:rounded-lg">
        <table className="min-w-full text-sm text-left text-gray-700 bg-white shadow-md rounded-lg">
            <thead className="text-xs uppercase bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
                <tr>
            ...
            <tbody>
                ...
                <tr key={payment.id} className="hover:bg-gray-50 transition-all duration-200 ease-in-out">
                    ...
                    <td className="px-6 py-4">
                        <span className={`inline-block px-4 py-2 text-sm font-semibold rounded-full
                            ${payment.status==="Diproses" ? "bg-yellow-100 text-yellow-800" :
                            payment.status==="Diterima" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800" }`}>
                            {payment.status}
                        </span>
                    </td>
    ...
    <div className="fixed inset-0 z-50 flex justify-center items-center bg-gray-800 bg-opacity-70">
        <div
            className="bg-white p-6 rounded-lg max-w-lg w-full relative shadow-xl transform transition-all duration-300 ease-in-out scale-95 hover:scale-100">
            <div className="flex justify-between items-center mb-6">
                ...
                <h3 className={`inline-block px-4 py-2 text-sm font-semibold rounded-full
                    ${selectedPayment.status==="Diproses" ? "bg-yellow-400 text-yellow-800" :
                    selectedPayment.status==="Diterima" ? "bg-green-200 text-green-800" : "bg-red-200 text-red-800" }`}>
                    {selectedPayment.status}
                </h3>
            </div>
            <div className="mb-6">
                <img src={`/bukti/${selectedPayment.proof}`} alt="Payment Proof"
                    className="w-full h-72 mb-4 object-cover rounded-lg shadow-md" />
                <div className="text-gray-700">
                    <div className="mb-2"><strong className="font-semibold">Nama Pengirim:</strong>
                        {selectedPayment.name}</div>
                    <div className="mb-2"><strong className="font-semibold">Bank:</strong>
                        {selectedPayment.bank.bankBrand}</div>
                    <div className="mb-2"><strong className="font-semibold">Nama Rekening:</strong>
                        {selectedPayment.bank.accountName}</div>
                    <div className="mb-2"><strong className="font-semibold">Nomor Rekening:</strong>
                        {selectedPayment.bank.accountNumber}</div>
                    <div className="mb-2"><strong className="font-semibold">Total:</strong>
                        {formatRupiah(selectedPayment.total)}</div>
                </div>
            </div>
            <div className="flex justify-between items-center">
                <button onClick={closeModal}
                    className="px-4 py-2 bg-gray-600 rounded-lg font-semibold text-white hover:bg-gray-700 transition-colors duration-200 ease-in-out">
                    Tutup
                </button>
                ...
                <div className="flex items-center gap-3 justify-start">
                    <button
                        className="bg-green-500 text-white px-5 py-2 rounded-lg hover:bg-green-600 transition-all duration-300 ease-in-out"
                        onClick={()=> handleModalAction("accept")}
                        >
                        Terima
                    </button>
                    <button
                        className="bg-red-500 text-white px-5 py-2 rounded-lg hover:bg-red-600 transition-all duration-300 ease-in-out"
