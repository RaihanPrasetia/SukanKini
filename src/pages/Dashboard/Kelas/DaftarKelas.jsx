    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-50 px-4">
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full overflow-hidden transform transition-all duration-300 scale-100">
        ...
        <div className="relative h-48 flex items-center justify-center">
          <img src={`/images/kelas/${classInfo.imagePath || "default-class.jpg"}`} alt={classInfo.name} className="w-full h-full object-cover rounded-t-lg" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/30 to-transparent" />
          <button onClick={onClose} className="absolute top-4 right-4 text-white bg-red-500  p-3 rounded-full shadow-lg hover:bg-white hover:text-red-600 hover:rotate-45 transition-all focus:outline-none">
            <AiOutlineClose size={18} />
          </button>
          <h2 className="text-2xl absolute font-bold text-white  text-center uppercase">{classInfo.name}</h2>
          <div className="flex flex-col justify-start items-center absolute p-4 bottom-0 left-0">
            <h2 className="text-lg font-semibold text-white  text-center">{classInfo.owner.name}</h2>
          </div>
        </div>
        ...
        <div className="p-4 ">
          ...
          <div className=" text-gray-700">
            <p className="text-lg"><strong className="font-semibold">Pelatih:</strong> {classInfo.trainer.name}</p>
            <p className="text-lg"><strong className="font-semibold">Alamat:</strong> {classInfo.address}</p>
            <p className="text-lg"><strong className="font-semibold">Jadwal:</strong> {classInfo.schedules.map((schedule) => `${schedule.hari} ${schedule.jam}`).join(", ")}</p>

            <div className="mb-2">
              <h5 className="font-semibold text-lg text-blue-500">Manfaat Kelas:</h5>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {classInfo.benefits.map((benefit, index) => (
                  <div key={benefit.id} className="p-4 bg-gray-50 rounded-lg shadow-md hover:shadow-lg transition-transform duration-200">
                    <h6 className="font-semibold text-green-600">{index + 1}. {benefit.name}</h6>
            ...
            <p className="text-2xl text-center text-green-500 font-bold"><strong className=" text-gray-600">Harga:</strong> Rp {classInfo.price.toLocaleString()}</p>
          </div>
          ...
          <div className="flex justify-center">
            <button onClick={handlePaymentClick} className="bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg hover:bg-green-700 transition-all transform hover:scale-105">
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-50 px-4">
          <div className="bg-white rounded-lg shadow-lg max-w-sm w-full p-6 animate-slide-up">
            ...
            >
              <option value="" disabled className="text-gray-400">-- Pilih Bank --</option>
              {bankList.map((bank) => (
                <option key={bank.id} value={bank.id}>{bank.brand}</option>
              ))}
            </select>
            ...
            {bankDetails && (
              <div className="mt-4 p-4 border border-green-200 bg-green-50 rounded-lg shadow-md">
                <p className="text-sm text-gray-700"><strong>Nomor Rekening:</strong> {bankDetails.no_rek}</p>
                <p className="text-sm text-gray-700"><strong>Atas Nama:</strong> {bankDetails.an}</p>
              </div>
            )}
            ...
            <div className="mt-4">
              ...
              <input
                id="bukti-pembayaran" type="file" accept="image/*"
            ...
            <div className="flex justify-between mt-6">
              <button onClick={() => setShowPaymentModal(false)} className="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition-all shadow-md">
                Batal
              </button>
              <button
