...
const PaymentPopup = ({
  ...
  const handlePayment = () => {
    Swal.fire({
      ...
      customClass: {
        popup:
          "rounded-lg shadow-lg bg-gradient-to-r from-green-300 to-blue-500",
        title: "font-bold text-green-800 text-xl",
        confirmButton:
          "bg-green-600 text-white hover:bg-green-700 font-medium py-2 px-4 rounded-lg",
      },
    }).then(() => {
      handleMenuClick();
  ...
  const showPaymentDetails = () => {
    Swal.fire({
      ...
      customClass: {
        htmlContainer:
          "swal2-html-container py-6 px-6 bg-white rounded-lg shadow-xl",
        input:
          "swal2-input w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 text-center",
        cancelButton:
