  ...
  useEffect(() => {
    const fetchClassDetails = async () => {
      try {
        ...
        console.log("Data kelas:", fetchedClass.class)
    ...
    <>
      <div className="flex flex-col pb-6 bg-gradient-to-r from-gray-100 via-gray-200 to-gray-300">
        ...
        <div className="flex flex-col md:flex-row px-6 md:px-20 py-8 space-y-6 md:space-y-0 md:space-x-8">
          {/* Image Section */}
          <div className="w-full md:w-1/2 items-center flex flex-col justify-center">
            <img
              src={`/images/kelas/${classInfo?.image_path}`}
              alt={classInfo?.name}
              className="w-64 h-64 md:w-80 md:h-80 rounded-full object-cover shadow-2xl transform transition-transform duration-300 ease-in-out hover:scale-105"
            />
          </div>
        ...
        <div className="mt-4 px-16">
          <h3 className="text-xl font-semibold text-gray-800 mb-2">Ulasan:</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {classInfo?.ratings?.map((rating) => (
              <div key={rating.id} className="flex items-center bg-white p-4 space-x-4 rounded-md">
                <img
                  src={`/images/profile/${rating.user.image_path}`}
                  alt={rating.user.name}
                  className="w-10 h-10 rounded-full mr-2"
                />
                <div>
                  <p className="text-gray-700 font-semibold">{rating.user.name}</p>
                  <p className="text-gray-600 text-sm">{rating.message}</p>
