        ...
        <div className="fixed inset-0 bg-gray-700 bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-8 rounded-xl w-11/12 sm:w-2/3 md:w-1/2 lg:w-1/3 space-y-6 shadow-xl transform transition-all animate__animated animate__zoomIn">
                ...
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-3xl font-semibold text-center text-green-600">Detail Pesanan</h3>
                    <div className="flex justify-center">
                        <span
                            className={`px-6 py-3 text-center font-semibold rounded-lg text-white ${order.paymentStatus === 'Diterima'
                                ? 'bg-gradient-to-r from-green-400 to-green-600 shadow-xl  transition-all duration-300'
                                : order.paymentStatus === 'Diproses'
                                    ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 shadow-xl  transition-all duration-300'
                                    : order.paymentStatus === 'Ditolak'
                                        ? 'bg-gradient-to-r from-red-400 to-red-600 shadow-xl  transition-all duration-300'
                                        : 'bg-gradient-to-r from-gray-400 to-gray-600 shadow-xl  transition-all duration-300'
                                }`}
                        >
                            {order.paymentStatus}
                        </span>
                ...
                <div className="flex justify-center w-full">
                    <img
                        ...
                        alt="Foto Pemesan"
                        className="w-full h-52 rounded-lg object-cover border-4 border-green-200 shadow-lg"
                    />
                </div>
                ...
                <div className="text-center mt-4">
                    <strong className="text-3xl text-green-600">Rp. {order.total.toLocaleString() || '0'}</strong>
                </div>

                {/* Close Button */}
                <div className="mt-6 text-center">
                    <button
                        className="px-6 py-2 bg-gray-400 text-white rounded-lg text-lg hover:bg-gray-500 transition duration-300 shadow-md"
                        onClick={closeModal}
                    >
                        Tutup
                    </button>
